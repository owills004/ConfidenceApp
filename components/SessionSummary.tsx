    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-fade-in">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden animate-scale-in flex flex-col max-h-[90vh]">
        ...
        <div className="p-6 overflow-y-auto">
          {activeTab === 'overview' && (
              <div className="space-y-6">
                <div className="bg-brand-50 border border-brand-100 p-4 rounded-xl flex items-center justify-between shadow-inner">
                    <div>
                        <div className="text-[10px] font-bold text-brand-500 uppercase tracking-widest mb-1">Hesitation Score</div>
                        <div className="text-3xl font-bold text-brand-900">{report.hesitationScore}</div>
                        <div className="text-[10px] text-brand-600 font-medium">Lower is better</div>
                    </div>
                    <div className={`w-12 h-12 rounded-full flex items-center justify-center text-white ${report.hesitationScore < 20 ? 'bg-green-500' : report.hesitationScore < 50 ? 'bg-yellow-500' : 'bg-red-500'}`}>
                        <i className={`fa-solid ${report.hesitationScore < 20 ? 'fa-check' : 'fa-triangle-exclamation'}`}></i>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                    <div className="bg-slate-50 p-3 rounded-xl border border-slate-100 text-center">
                        ...
                        <div className="text-xl font-bold text-slate-700">{report.fillerWordCount}</div>
                    </div>
                    <div className="bg-slate-50 p-3 rounded-xl border border-slate-100 text-center">
                        <div className="text-slate-500 text-[10px] uppercase font-bold tracking-wider mb-1">Awkward Pauses</div>
                        <div className="text-xl font-bold text-slate-700">{report.awkwardPauseCount}</div>
                    </div>
                    <div className="bg-slate-50 p-3 rounded-xl border border-slate-100 text-center">
                    ...
                    <div className="bg-slate-50 p-3 rounded-xl border border-slate-100 text-center">
                        <div className="text-slate-500 text-[10px] uppercase font-bold tracking-wider mb-1">Tone</div>
                        <div className="text-xl font-bold text-slate-700 truncate px-1">{report.dominantEmotion}</div>
                    </div>
                </div>

                {audioBlob && (
                  <button onClick={handleDownloadAudio} className="w-full flex items-center justify-center gap-2 py-4 bg-slate-900 text-white rounded-xl font-bold hover:bg-slate-800 transition-all shadow-lg active:scale-95">
                ...
                {suggestions.length > 0 && (
                    <div className="mt-8">
                        ...
                        <div className="space-y-2">
                            {suggestions.map((s) => (
                                <button key={s.id} onClick={() => onSelectScenario?.(s)} className="w-full flex items-center gap-4 p-3 bg-white border border-slate-200 rounded-xl hover:border-brand-300 hover:bg-brand-50/30 transition-all text-left group">
                                    <div className={`w-8 h-8 rounded-lg ${s.color} text-white flex items-center justify-center shrink-0 shadow-sm`}>
                                        <i className={`fa-solid ${s.icon} text-xs`}></i>
                                    </div>
                                    <div className="flex-1 min-w-0">
                                        <h5 className="text-xs font-bold text-slate-900 group-hover:text-brand-700">{s.title}</h5>
                                    </div>
                                    <i className="fa-solid fa-chevron-right text-slate-300 group-hover:text-brand-400 text-[10px]"></i>
                                </button>
                            ))}
                        </div>
                    </div>
                      ...
                      report.grammarCorrections.map((item, idx) => (
                          <div key={idx} className="bg-slate-50 p-4 rounded-xl border border-slate-200">
                              <div className="mb-3 pb-3 border-b border-slate-200/50">
                                  ...
                                  <p className="text-slate-700 line-through opacity-70 italic font-serif">"{item.original}"</p>
                              </div>
                              <div className="mb-3">
                              ...
                              <div className="flex items-start gap-2 bg-blue-50/50 p-2 rounded-lg">
                                  <span className="bg-blue-100 text-blue-700 text-[10px] font-bold px-2 py-0.5 rounded uppercase tracking-wide mt-0.5">Rule</span>
                  ...
                  {report.confidenceTips && report.confidenceTips.length > 0 ? (
                      report.confidenceTips.map((tip, idx) => (
                          <div key={idx} className="flex gap-4 p-4 bg-purple-50 rounded-xl border border-purple-100">
                              <div className="shrink-0 w-8 h-8 bg-purple-200 text-purple-700 rounded-full flex items-center justify-center font-bold">{idx + 1}</div>
                              <div>
                                  <h4 className="font-bold text-purple-900 text-sm mb-1">Coaching Insight</h4>
                                  <p className="text-sm text-purple-800/80 leading-relaxed">{tip}</p>
                              </div>
                          </div>
                      ))
                  ) : (
                      <div className="text-center py-8 text-slate-400"><p>No specific tips available.</p></div>
                  )}
              </div>
          )}
          {activeTab === 'transcript' && (
             <div className="space-y-4">
               {report.transcript && report.transcript.length > 0 ? (
                 ...
                 <div className="text-center py-8 text-slate-400"><p>No transcript available.</p></div>
               )}
             </div>
          )}
        </div>
        ...
        <div className="p-4 border-t border-slate-100 bg-slate-50 shrink-0">
            <button onClick={onClose} className="w-full py-3 bg-brand-600 text-white rounded-xl font-bold hover:bg-brand-700 shadow-lg">Back to Dashboard</button>
        </div>
      </div>
    </div>
  );
